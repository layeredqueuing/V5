...
{
    if ( amt == 0 ) return *this;		/* No operation.		*/
 ...
 * Alas, since we have a cltn with non pointer types, we can't
 * include this function as a member of type Cltn (some compilers
 * barf.)
 */
...
/*
 * Sort the collection.
 * NOTE: since we play games with the alignment of the arrays, we can't use quicksort on sparcs.
 */

template <class Type> Cltn<Type> const&
Cltn<Type>::sort( compare_func_ptr compare, const unsigned size ) const
{
    if ( ia && sz > 1 ) {
#if defined(__sparc)
	for ( unsigned i = ::min(sz,size); i > 0; --i ) {
...
#else
	::qsort( (void *)&ia[1], (size_t)::min( sz, size ), sizeof(Type *), compare );
#endif
    }
    return *this;
}
...
template <class Type> Cltn<Type> const&
Cltn<Type>::swap( const unsigned i, const unsigned j ) const
{
...
template <class Type> ostream& 
Cltn<Type>::print( ostream& os ) const
{
...
/*
 * Check for valid bounds and initialize.
 * myMin can be larger than myMax, in which case this class is a nop.
 */
