...
\lstdefinelanguage{LQN}{
  basicstyle=\ttfamily,
  keywords={G,P,U,T,E,A,R,C,p,g,t,s,y,c,f,z,F,-1,->,\#pragma},
  classoffset=1,
...
\lstdefinelanguage{LQX}{
  keywords={foreach,in},
  keywords={[2]solve,println,task,processor,entry,throughput,utilization},
...
m_client = [1, 2, 3];                         	// $m_client = [1,2,3]
m_server = [2, 3, 4];                         	// $m_server = [2,3,4]
foreach( $m_client in m_client ) {
  foreach( $m_server in m_server ) {
    $s_server = $m_client / 2;                	// $s_server = $m_client / 2;
    solve();
    $f_client = task("client").throughput;	// %f $f_client
    $u_server = task("server").utilization;     // %u $u_server
