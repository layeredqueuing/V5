...
#if !defined(WSPN_OBJECT_H)
#define WSPN_OBJECT_H
#include "global.h"
...
#ifdef	NOTUSED
struct images			{ struct pixrect 	*i_p;
				  float			 i_top;
				  float			 i_left;
				  float			 i_width;
				  float			 i_height;
...
struct probability		{ float			 prob;
				  short			 val;
				  struct probability	*next;
				};

struct lisp_object		{ char			*tag;
				  struct position	 center;
				  char			 type;
...
struct tok_object               { struct tarc           *larc;
				  struct  tarc          *pas;
				  struct  tarc          *jum;
				  struct  tarc          *mpl;
...
struct tarc                   { float                 arr;
                                struct tarc          *next;
                              };


struct text_object		{ int			 font, size, style;
				  int			 height, length;
...
struct net_object		{ struct com_object	*comment;
