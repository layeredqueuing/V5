...
namespace LQX {
  namespace Strings {
    ...
    SymbolAutoRef str_cmp::invoke(Environment* env, std::vector<SymbolAutoRef >& args) throw (RuntimeException)
    {
  ...
  namespace Strings {
    ...
    /* This method on the other hand actually does all the heavy lifting */
    SymbolAutoRef str_concat::invoke(Environment* env, std::vector<SymbolAutoRef >& args) throw (RuntimeException)
    {
      /* Pass the arguments up to strcmp */
      const char* s1 = decodeString(args, 0);
      const char* s2 = decodeString(args, 1);
